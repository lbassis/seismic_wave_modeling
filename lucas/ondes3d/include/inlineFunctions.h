...
/*============= Place Functions=========================== */
static inline enum typePlace WhereAmI(int i, int j, int k,
					   ...
					   enum typeModel model)
{
    ...
    return (8. /
	    (1. / value1 + 1. / value2 + 1. / value3 + 1. / value4 +
	     1. / value5 + 1. / value6 + 1. / value7 + 1. / value8));

}

static inline double averageInverseDouble4(double value1, double value2,
...
{				/* Status : Used */
    assert((model == LAYER || model == GEOLOGICAL));
    if (model == LAYER) {
	assert(value1 > PRECISION_INVERSE && value2 > PRECISION_INVERSE);
    } else if (model == GEOLOGICAL) {
	if (value1 <= PRECISION_INVERSE || value2 <= PRECISION_INVERSE) {
	    return 0.;
	}
    }
...
static inline double CornerXYZ_Geol(double *field,
				    int i, int j, int k, struct MEDIUM MDM)
{
    int idum, jdum, kdum;
    double res = 0.;
    int med;
...
static inline double staggardv4(double b,
				...
				double kappaz, double dt, double dx,
				...
				enum typePlace place, enum typePML method)
{
	...
	return (9. * b * dt / 8.) * ((x2 - x1) / kappax +
				     (y2 - y1) / kappay + (z2 -
							   z1) / kappaz) /
	    dx - (b * dt / 24.) * ((x4 - x3) / kappax +
				   (y4 - y3) / kappay + (z4 -
							 z3) / kappaz) /
	    dx;
    } else {			/* usual */
				...
				enum typePlace place, enum typePML method)
{
	...
	if (method == CPML) {
	    return (9. * dt / 8.) * ((kap + 4. * mu / 3.) * (x2 - x1) /
				     kappax + (kap - 2. * mu / 3.) * (y2 -
								      y1) /
				     kappay + (kap - 2. * mu / 3.) * (z2 -
								      z1) /
				     kappaz) / dx - (dt / 24.) * ((kap +
								   4. *
								   mu /
	...
	return (9. * dt / 8.) * ((kap + 4. * mu / 3.) * (x2 - x1) +
				 (kap - 2. * mu / 3.) * (y2 - y1 + z2 -
							 z1)) / dx -
	    (dt / 24.) * ((kap + 4. * mu / 3.) * (x4 - x3) +
			  (kap - 2. * mu / 3.) * (y4 - y3 + z4 - z3)) / dx;
    }
}

static inline double staggardt4(double mu,
				double kappax, double kappay, double dt,
				double dx, double x1, double x2, double x3,
				double x4, double y1, double y2, double y3,
				double y4, enum typePlace place,
				enum typePML method)
{
    if ((place == ABSORBINGLAYER || place == FREEABS)) {
	if (method == CPML) {
	    return (9. * dt * mu / 8.) * ((x2 - x1) / kappax +
					  (y2 - y1) / kappay) / dx -
		(dt * mu / 24.) * ((x4 - x3) / kappax +
				   (y4 - y3) / kappay) / dx;
	} else if (method == PML) {
	    return 0.;
	}
...
{
    return cos(rake) * cos(dip) * cos(strike) +
	sin(rake) * cos(2. * dip) * sin(strike);
}

static inline double radxz(double strike, double dip, double rake)
{
    return cos(rake) * cos(dip) * sin(strike) -
	sin(rake) * cos(2. * dip) * cos(strike);
}
			      ...
			      double x4)
{
    return ((2. - dt * dump) * v +
	    b * ((9. / 8.) * (x2 - x1) -
		 (1. / 24.) * (x4 - x3)) * (2. * dt / dx)) / (2. +
							      dt * dump);
}

static inline double CPML4(double vp, double dump, double alpha,
			   double kappa, double phidum, double dx,
			   double dt, double x1, double x2, double x3,
			   double x4)
{
    double a, b;
...
static inline double FKSIs4(double mu, double kap, double dx,
			    ...
			    double z1, double z2, double z3, double z4)
{
    return 2. * mu * As * ((9. / 8.) * (x2 - x1) -
			   (1. / 24.) * (x4 - x3)) / dx + ((kap +
							    (4. / 3.) *
							    mu) * Ap -
							   2. * mu * As) *
	((9. / 8.) * (x2 - x1) - (1. / 24.) * (x4 - x3) +
	 (9. / 8.) * (y2 - y1) - (1. / 24.) * (y4 - y3) + (9. / 8.) * (z2 -
								       z1)
	 - (1. / 24.) * (z4 - z3)) / dx;
}
...
static inline double FKSIs4CPML(double mu, double kap, double dx,
				...
				double phizdum, double dumpx,
				double alphax, double kappax, double dumpy,
				double alphay, double kappay, double dumpz,
				double alphaz, double kappaz, double x1,
				double x2, double x3, double x4, double y1,
				double y2, double y3, double y4, double z1,
				double z2, double z3, double z4)
{
    ...
    a_x = 0.0;
    if (abs(vp * dumpx) > 0.000001)
	a_x =
	    vp * dumpx * (b_x -
			  1.0) / (kappax * (vp * dumpx + kappax * alphax));

    b_y = exp(-(vp * dumpy / kappay + alphay) * dt);
    a_y = 0.0;
    if (abs(vp * dumpy) > 0.000001)
	a_y =
	    vp * dumpy * (b_y -
			  1.0) / (kappay * (vp * dumpy + kappay * alphay));

    b_z = exp(-(vp * dumpz / kappaz + alphaz) * dt);
    a_z = 0.0;
    if (abs(vp * dumpz) > 0.000001)
	a_z =
	    vp * dumpz * (b_z -
			  1.0) / (kappaz * (vp * dumpz + kappaz * alphaz));

    dxt =
	b_x * phixdum + (1. / kappax + a_x) * ((9. / 8.) * (x2 - x1) / dx -
					       (1. / 24.) * (x4 -
							     x3) / dx);
    dyt =
	b_y * phiydum + (1. / kappay + a_y) * ((9. / 8.) * (y2 - y1) / dx -
					       (1. / 24.) * (y4 -
							     y3) / dx);
    dzt =
	b_z * phizdum + (1. / kappaz + a_z) * ((9. / 8.) * (z2 - z1) / dx -
					       (1. / 24.) * (z4 -
							     z3) / dx);

    return 2. * mu * As * dxt + ((kap + (4. / 3.) * mu) * Ap -
				 2. * mu * As) * (dxt + dyt + dzt);
}


static inline double FKSIt4CPML(double mu, double dx, double dt, double As,
				double vp, double phixdum, double phiydum,
...
/* KRISTEK AND MOCZO */
static inline double Diff4(double dx, double x1, double x2, double x3,
			   double x4)
{
...
void test_WhereAmI(void)
{
    ...
    printf
	(",: Regular, # :AbsorbingLayer,- : FreeSurface,| : Limit, . : OutSide : :FREEABS\n");
    for (j = PRM.yMin - PRM.delta; j <= PRM.yMax + PRM.delta + 2; j++) {
	printf("\ntest of xz plane at y=%i\n", j);
	printf("Absorbing \t\t\tFree \n", j);
	for (k = PRM.zMax + PRM.delta + 3; k >= PRM.zMin - PRM.delta - 1;
	     k--) {
	    PRM.zMax0 = PRM.zMax + PRM.delta + 2;	/* Absorbing */
	    for (i = PRM.xMin - PRM.delta - 1;
		 i <= PRM.xMax + PRM.delta + 3; i++) {
