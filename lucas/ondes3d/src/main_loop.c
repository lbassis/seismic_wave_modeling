void main_loop(struct PARAMETERS *PRM, ) {
  ...
  struct starpu_data_filter x_filter, y_filter;
  x_filter = {.filter_func = starpu_block_filter_block, .nchildren = n_blocks_x};
  y_filter = {.filter_func = starpu_block_filter_vertical_block,.nchildren = n_blocks_y};
  ...
  starpu_matrix_data_register(&src_vel_handle, STARPU_MAIN_RAM, (uintptr_t)SRC->vel, SRC->iDur, SRC->iDur, SRC->iSrc, sizeof(SRC->vel[0][0]));
  starpu_matrix_vector_register(&src_strike_handle, STARPU_MAIN_RAM, (uintptr_t)SRC->strike, SRC->iSrc, sizeof(SRC->strike[0]));
  starpu_matrix_vector_register(&src_dip_handle, STARPU_MAIN_RAM, (uintptr_t)SRC->dip, SRC->iSrc, sizeof(SRC->dip[0]));
  starpu_matrix_vector_register(&src_rake_handle, STARPU_MAIN_RAM, (uintptr_t)SRC->rake, SRC->iSrc, sizeof(SRC->rake[0]));
  starpu_matrix_vector_register(&src_xweight_handle, STARPU_MAIN_RAM, (uintptr_t)SRC->xweight, SRC->iSrc, sizeof(SRC->xweight[0]));
  starpu_matrix_vector_register(&src_yweight_handle, STARPU_MAIN_RAM, (uintptr_t)SRC->yweight, SRC->iSrc, sizeof(SRC->yweight[0]));
  starpu_matrix_vector_register(&src_zweight_handle, STARPU_MAIN_RAM, (uintptr_t)SRC->zweight, SRC->iSrc, sizeof(SRC->zweight[0]));
  starpu_matrix_vector_register(&src_insrc_handle, STARPU_MAIN_RAM, (uintptr_t)SRC->insrc, SRC->iSrc, sizeof(SRC->insrc[0]));
  starpu_matrix_vector_register(&src_ixhypo_handle, STARPU_MAIN_RAM, (uintptr_t)SRC->ixhypo, SRC->iSrc, sizeof(SRC->ixhypo[0]));
  starpu_matrix_vector_register(&src_iyhypo_handle, STARPU_MAIN_RAM, (uintptr_t)SRC->iyhypo, SRC->iSrc, sizeof(SRC->iyhypo[0]));
  starpu_matrix_vector_register(&src_izhypo_handle, STARPU_MAIN_RAM, (uintptr_t)SRC->izhypo, SRC->iSrc, sizeof(SRC->izhypo[0]));
  starpu_matrix_vector_register(&prm_i2imp_handle, STARPU_MAIN_RAM, (uintptr_t)PRM->i2imp_array, ((PRM->xMax+2*PRM->delta+2) - (PRM->xMin - PRM->delta) + 1) , sizeof(PRM->i2imp_array[0]));
  starpu_matrix_vector_register(&prm_j2jmp_handle, STARPU_MAIN_RAM, (uintptr_t)PRM->j2jmp_array, ((PRM->yMax+2*PRM->delta+2) - (PRM->yMin - PRM->delta) + 1) , sizeof(PRM->j2jmp_array[0]));

  starpu_block_vector_register(&src_fx_handle, STARPU_MAIN_RAM, (uintptr_t)&(SRC->fx), nrows*ncols*depth, sizeof(SRC->fx[0][0][0]));
  starpu_block_vector_register(&src_fy_handle, STARPU_MAIN_RAM, (uintptr_t)&(SRC->fy), nrows*ncols*depth, sizeof(SRC->fy[0][0][0]));
  starpu_block_vector_register(&src_fz_handle, STARPU_MAIN_RAM, (uintptr_t)&(SRC->fz), nrows*ncols*depth, sizeof(SRC->fz[0][0][0]));
  ...
  starpu_data_handle_t phivxx_handle, phivyy_handle, phivzz_handle, phivyx_handle, phivxy_handle, phivzx_handle, phivzx_handle, phivxz_handle, phivzy_handle;
  ...
  starpu_vector_data_register(&dumpx_handle,  STARPU_MAIN_RAM, (uintptr_t)ABC->dumpx,  mpmx, sizeof(ABC->dumpx[0]));
  starpu_vector_data_register(&dumpx2_handle, STARPU_MAIN_RAM, (uintptr_t)ABC->dumpx2, mpmx, sizeof(ABC->dumpx[0]));
  starpu_vector_data_register(&dumpy_handle,  STARPU_MAIN_RAM, (uintptr_t)ABC->dumpy,  mpmy, sizeof(ABC->dumpx[0]));
  starpu_vector_data_register(&dumpy2_handle, STARPU_MAIN_RAM, (uintptr_t)ABC->dumpy2, mpmy, sizeof(ABC->dumpx[0]));
  ...
  starpu_vector_data_register(&alphax_handle,  STARPU_MAIN_RAM, (uintptr_t)ABC->alphax,  mpmx, sizeof(ABC->alphax[0]));
  starpu_vector_data_register(&alphax2_handle, STARPU_MAIN_RAM, (uintptr_t)ABC->alphax2, mpmx, sizeof(ABC->alphax[0]));
  starpu_vector_data_register(&alphay_handle,  STARPU_MAIN_RAM, (uintptr_t)ABC->alphay,  mpmy, sizeof(ABC->alphax[0]));
  starpu_vector_data_register(&alphay2_handle, STARPU_MAIN_RAM, (uintptr_t)ABC->alphay2, mpmy, sizeof(ABC->alphax[0]));
  ...
  starpu_vector_data_register(&kappax_handle,  STARPU_MAIN_RAM, (uintptr_t)ABC->kappax,  mpmx, sizeof(ABC->kappax[0]));
  starpu_vector_data_register(&kappax2_handle, STARPU_MAIN_RAM, (uintptr_t)ABC->kappax2, mpmx, sizeof(ABC->kappax[0]));
  starpu_vector_data_register(&kappay_handle,  STARPU_MAIN_RAM, (uintptr_t)ABC->kappay,  mpmy, sizeof(ABC->kappax[0]));
  starpu_vector_data_register(&kappay2_handle, STARPU_MAIN_RAM, (uintptr_t)ABC->kappay2, mpmy, sizeof(ABC->kappax[0]));
  ...
  starpu_variable_block_register(&v0_x_handle, STARPU_MAIN_RAM, (uintptr_t)&(v0->x), nrows*ncols*depth, sizeof(v0->x[0][0][0]));
  starpu_variable_block_register(&v0_y_handle, STARPU_MAIN_RAM, (uintptr_t)&(v0->y), nrows*ncols*depth, sizeof(v0->y[0][0][0]));
  starpu_variable_block_register(&v0_z_handle, STARPU_MAIN_RAM, (uintptr_t)&(v0->z), nrows*ncols*depth, sizeof(v0->z[0][0][0]));
  ...
  starpu_data_handle t0_xx_handle, t0_yy_handle, t0_zz_handle, t0_xy_handle, t0_xz_handle, t0_yz_handle;

  starpu_variable_block_register(&t0_xx_handle, STARPU_MAIN_RAM, (uintptr_t)&(t0->xx), nrows*ncols*depth, sizeof(t0->xx[0][0][0]));
  starpu_variable_block_register(&t0_yy_handle, STARPU_MAIN_RAM, (uintptr_t)&(t0->yy), nrows*ncols*depth, sizeof(t0->yy[0][0][0]));
  starpu_variable_block_register(&t0_zz_handle, STARPU_MAIN_RAM, (uintptr_t)&(t0->zz), nrows*ncols*depth, sizeof(t0->zz[0][0][0]));
  starpu_variable_block_register(&t0_xy_handle, STARPU_MAIN_RAM, (uintptr_t)&(t0->xy), nrows*ncols*depth, sizeof(t0->xy[0][0][0]));
  starpu_variable_block_register(&t0_xz_handle, STARPU_MAIN_RAM, (uintptr_t)&(t0->xz), nrows*ncols*depth, sizeof(t0->xz[0][0][0]));
  starpu_variable_block_register(&t0_yz_handle, STARPU_MAIN_RAM, (uintptr_t)&(t0->yz), nrows*ncols*depth, sizeof(t0->yz[0][0][0]));
  ...
  // v0 eh de boas, t0 precisa dos vizinhos
  ...
  for (i_block = 1; i_block <= n_blocks_y; i_block++) {
    for (j_block = 1; j_block <= n_blocks_x; j_block++) {
      ...
      starpu_vector_data_register(&ipml_handle, STARPU_MAIN_RAM, (uintptr_t)ABC->ipml[i][j], sizeof(ABC->ipml[i][j]));
      
      // muito questionavel
      long int first_npml = ABC->ipml[i][j][0];
      ...
      starpu_task_insert(&intermediates_cl,
			 ...
			 STARPU_VALUE, &imp, sizeof(imp),
			 STARPU_VALUE, &jmp, sizeof(jmp),
