...
Apesar do grande interesse desse estudo em realizar uma comparação da nova implementação com a antiga, não foi possível concluir o desenvolvimento da nova versão utilizando
...
a mais complexa. Por algum motivo ainda não identificado, o tensor de mapeamento $ipml$ é consultado durante o cálculo de células que não se encontram nas bordas, fornecendo
...
uma análise mais profunda é necessária para detectar se o erro encontra-se no tensor $ipml$ ou nos vetores por ele mapeados.
...
das dependências entre as tarefas de computação do momento sísmico e de velocidade demonstrarem estar corretas sugere que existe alguma particularidade no tratamento das tarefas incorretamente
...
foram detectadas inconsistências entre as dependências. Independentemente desses resultados, os estudos elaborados a respeito do funcionamento da aplicação servem de base para a elaboração de
