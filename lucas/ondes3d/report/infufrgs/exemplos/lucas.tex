...
% introducao
\chapter{Introdução}
\todo{``calcula diferenes grandezas físicas'' além do erro
  ``diferenes'', fica estranho como
  tradução. Procure por método dos elementos finitos (wikipedia)
  para uma tradução melhor. Embora }
\todo{``diversos trabalhos'' -> coloque as citações imediatamente}
\todo{``alguns deles apresentados no capítulo 2'' -> remover, e
  questione-se porque somente ``alguns''. Idealmente, fale sobre todos
  até onde teu (nosso) conhecimento alcança.}
...
\todo{Confirme o que o Ondes3D utiliza, se é MEF ou se é MDF. Para
  saber a diferença entre eles, encontrei esse trabalho:
  https://aguassubterraneas.abas.org/asubterraneas/article/view/29636/19134
  onde no PDF, Pag2, Sec2, primeiro parágrafo, tu tens explicitamente
  a diferença (assumindo que eles tem a definição correta). Nesse
  sentido, está correto dizer que usa-se MDF (pois ondes3D tem divisão
regular), e no parágrafo anterior deve-se atualizar em função disso.}
...
\todo{``sem a possibilidade de entrelaçar a computação de diferentes
  estruturas'' -> entre diferentes etapas.}
\todo{ Verificar acentuação em ``concluirem''.}
...
\todo{Isso ``maior quantidade de tarefas melhorando a distribuição da
  carga'' (por causa do escalonamento) o que desejamos obter. Nossa
  hipótese. Nota que isso é por verificar. Talvez deixar isso mais claro.}
...
\todo{A construção do grafo não é explícita e não é feita. Embora
  possa-se exportar. O StarPU nunca vê todo o grafo. Talvez
  simplesmente substituir o verbo ``para construir'' seja suficiente.}
...
\todo{Frase muito grande, quebrá-la.}
...
\todo{Par. de uma frase muito grande. Quebrá-la.}
\todo{``de ondas'' -> de ondas sísmicas.}
...
\todo{``Apesar da versão implementada atualmente não ser capaz de
  simular corretamente todos os fenômenos físicos envolvidos na
  simulação'' -> complementar informando que isso é conforme os
  experimentos realizados comparando o resultado das simulações entre
  a versão de referência e a versão implementada. Talvez seja
  importante tu tentar descobrir para apontar onde possivelmente
  encontra-se o equívoco, ou ao menos levantar algumas pistas.}
...
\todo{Para trabalhos futuros ao invés de ``eventual sequência do
...
\todo{Colocar aqui uma section chamada ``Estrutura do texto'' e dentro
  dela adicionar um único parágrafo onde tu apresentas a estrutura do documento.}

\chapter{Trabalhos relacionados}\label{sec:related}

\todo{``são discutidos abaixo'' -> complementar com ``e as diferenças com o presente trabalho são explicitadas.''} 
...
\todo{Também informar que embora tu citas Boito et al e seu trabalho
  de IO, em todos os teus experimentos as atividades de entrada e
  saída são desconsideradas para que o enfoque seja exclusivamente no
  tempo de cálculo. Por conta disso, na versão que tu otimizas, não há
  saída (pois a entrada são os estudos de caso). Podes também dizer
  que teu trabalho iria beneficiar igualmente pois poderia haver
  tarefas de IO a serem executadas de maneira assíncrona (que não
  prendem a continuidade da simulação), embora isso esteja fora do
  escopo do teu trabalho.}
...
\todo{depende da região sendo tratada mas também dos valores da
  simulação (a onda sísmica em si) tendo portanto um comportamento
  dinâmico (no tempo) além daquele do espaço já conhecido.}
\todo{Tesser permite sim usar programação distribuída - ele usam o
  AMPI, layer MPI sob o Charm++. A vantagem do uso do AMPI é que ele
  permite a migração de um processo MPI de um computador para outro,
  resolvendo o problema do desbalanceamento da carga.}
...
\todo{Qual processador (explicitar na primira frase).}
\todo{Além de não considerado, a forma de paralelização multi-nível
  deste trabalho é bastante diferente do que tu propões aqui. Veja que
na delas, é praticamente uma abordagem MPI+OpenMP.}
...
\todo{Ler o trabalho do Nesi para saber e expliciar aqui qual é a aplicação de dinâmica
  de fluídos.}
\todo{Citar o trabalho do StarVZ. Veja ``Origin and Publications''
  aqui https://github.com/schnorr/starvz}
\todo{Explicada na Seção (maiuscula).}
\todo{Talvez explicitar que tu és tu que explicas.... em ``explicada
  na seção \ref{sec:proposal}''.}
...
\todo{Qual a aplicação!?!? Dizer que aqui voltaste a falar do Ondes3D
  (pois o parágrafo anterior não o era)}
\todo{Optaram por descartar ... revisar pt-br. Podes dizer que na tua
  versão tu não faz esse descarte.}
\todo{Performance é uma palavra que não existe em pt-br, usar
  ``desempenho''.}
\todo{Nota que o ganho de desempenho relatado é contra uma versão que
  faz mais coisa (se em dúvidas, confirma isso no trabalho do Victor).}
...
\todo{entre \textbf{o} trabalho desenvolvido e descrito neste
  documento...}
\todo{Ao invés de escrever Nesi et al. e...  citar os trabalhos com \\cite} 
...
\todo{Empregar os pacotes booktabs e tabularx (colocar a última coluna
  com X). Não usar linhas verticais na
  tabela. Sobre booktabs, veja
https://nhigham.com/2019/11/19/better-latex-tables-with-booktabs/
Sobre tabularx, force que a largura da tabela ocupe toda a largura
disponível no documento (respeitando as margens habituais).}
\todo{Substituir ``Dinâmica de fluídos'' pelo nome da aplicação.}
\begin{table}[htb!]
...
\todo{Achei que ficou estranha a passagem ``é armazenada em uma
  estrutura a parte que possui múltiplos tensores'' talvez porque a
  frase ficou muito muito grande.}
\todo{Lembra que seção deve ser referenciada em maiúsculo. Lembrando
  novamente então.}
...
\todo{Mencionar que cada iteração é um passo de tempo, um delta,
  especificado em arquivo. O delta de tempo se me recordo bem é de
  0.1s, então em uma iteração é simulada a prop. da onda em 0.1s
  naquele meio. Isso pode ajudar a entender o que são as iterações.}
...
\todo{A aplicação Ondes3D \textbf{implementa} as duas ...}
...
\todo{Não fica claro ao que tu te refere ``de forma
  parametrizável''. Veja que isso tu podes elaborar depois então a
  sugestão é de remover esse aposto. Podes alternativamente, no final
  desta frase, adicionar outra, informando que o tamanho dos blocos
  será configurável pelo usuário do programa de maneira a adaptá-lo na
  parte experimental.}
\todo{Peraí, a seção diz ``Proposta'' mas aqui tu traz a forma como a
  versão original discretiza o problema em blocos. Não é a tua versão então?}
...
\todo{Aqui novamente fala-se da versão original. Essas descrições da
  versão original
  (blocos acima e iterações abaixo) não fariam mais sentido na Sec 3.1
  Ondes3D?}
...
\todo{Okay, aqui tu começas a descrever a tua proposta.}
\todo{``assim permitirem'' -> estiverem satisfeitas.}
...
\todo{Ficou estranho esse ``na verdade''. Queres dizer que o resto não
  é verdade?}
\todo{Atenção para o pt-br aí hein, ``de cada tarefa utilizar...''}
...
\todo{Sobre o aumento do uso de memória, veja que o StarPU e o
  paralelismo natural do DAG já limitam a memória tendo em vista que
  no máximo N tarefas estarão executando efetivamente em paralelo
  sendo N a quantidade de workers.} 
...
\todo{OK. Tu explicas enfim células fantasmas, mas tu usas? Faz parte
  da tua proposta? Ahh, do projeto, okay.} 
...
\todo{Começa direto falando das dificuldades... quais dificuldades?
  Ainda não chegamos no drama delas. Talvez possa ser mais positivo,
  falando as possibilidades ao invés das dificuldades.}
\todo{A fim de -> Com o objetivo de}
...
\todo{Na frase que começa por ``No entanto,'', senti que faltou a
  dualidade logo antes de ``e as funções''. Minha sugestão é trocar o
  ``e'' por um ``mas'', indicando a contrariedade existente.}
...
\todo{link quebrado em \ref{sec:tasks}. Corrigir.}
\todo{seção -> Seção}
\todo{Penso que esta parte ficou incompleta tendo em vista que a
  ref. para a seção com mais detalhes não está correta. Faltaram
  portanto os detalhes. Aonde
  exatamente aquela ref está apontando para?}
...
\todo{Já tinha ``sentido'' isso antes: será que não daria para ter uma
  tradução para as condições de absorção nas bordas?}
...
\todo{o vetor phivx é uma matriz.}
...
\todo{Creio que  \textit{phiv\_t} está repetido duas vezes. A na
  figura temos  \textit{phiv\_s} ao invés de \textit{phit\_t} conforme
  mencionado no texto.}
...
\todo{Ao ler até aqui, tenho o feeling que o texto está bem técnico
  com esses detalhes (na linha do que tu falaste que é um trabalho de
  implementação técnica). Acho que para dá para aliviar nessa
  densidade técnica, tentando descrever melhor os elementos da figura,
como por exemplo o porque de 2* 3* 4*...}
\todo{Parágrafo só tem uma frase.}
...
\todo{Acho que não tem crase em ``à um bloco cada um''}
...
\todo{Usar mais a Figura 4.9 para explicar tendo em vista que a sua
  menção aparece apenas no final.}
...
\todo{Minha sugestão é remover a numeração desta subsubsection (porque
  ela é única, não há outra subsubsection). Para
  tal basta usar * depois de subsubsection antes do \{.}
