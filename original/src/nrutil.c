...
and initiate 0*/
{
    ...
    /* allocate rows and set pointers to them */
    m[nrl] =
	(double *) calloc((nrow * ncol + NR_END),
...
/*         t=(double ***) malloc((size_t)((nrow+NR_END)*sizeof(double**))); */
    t = malloc((size_t) ((nrow + NR_END) * sizeof(double **)));
    if (!t)
...
/*         t[nrl]=(double **) malloc((size_t)((nrow*ncol+NR_END)*sizeof(double*))); */
    t[nrl] = malloc((size_t) ((nrow * ncol + NR_END) * sizeof(double *)));
    if (!t[nrl])
...
/* allocate a double 3tensor with range t[nrl..nrh][ncl..nch][ndl..ndh] */
{
    ...
    if (!t[nrl][ncl])
	nrerror("allocation failure 3 in i3tensor()");
    t[nrl][ncl] += NR_END;
    t[nrl][ncl] -= ndl;
...
and initiate 0*/
{
    ...
    /* allocate rows and set pointers to them */
    t[nrl][ncl] =
	(double *) calloc((nrow * ncol * ndep + NR_END),
			  (size_t) (sizeof(double)));
    if (!t[nrl][ncl])
	nrerror("allocation failure 3 in f3tensor()");
    t[nrl][ncl] += NR_END;
