...
// constant mem {{{
   ...
   {
      printCudaErr(cudaMemcpyToSymbol(dump0, &d0, sizeof(float)),"__FUNCTION__");
      printCudaErr(cudaMemcpyToSymbol(kappa0, &k0, sizeof(float)),"__FUNCTION__");
      printCudaErr(cudaMemcpyToSymbol(alpha0, &a0, sizeof(float)),"__FUNCTION__");
   }
...
// DEVICE FUNCTIONS {{{
   // previously CPMLS arrays in textures {{{
      ...
      {
            float abscissa_normalized = MAX((ixe_min - i)/(float)DELTA, (i - (ixs_max-1)/(float)DELTA));

            return (abscissa_normalized>=0)?dump0*powf(abscissa_normalized,NPOWER):0.0;
      }
      inline __device__ float dumpy(int i)
      {
            float abscissa_normalized = MAX((iye_min - i)/(float)DELTA, (i - (iys_max-1)/(float)DELTA));

            return (abscissa_normalized>=0)?dump0*powf(abscissa_normalized,NPOWER):0.0;
      }
      inline __device__ float dumpz(int i)
      {
            float abscissa_normalized = (DELTA - i)/(float)DELTA;
      ...
      inline __device__ float kappax(int i)
      {
            ...
            return (abscissa_normalized>=0)?1.0 + (kappa0 - 1.0) * powf(abscissa_normalized,NPOWER):1.0;
      }
      ...
      inline __device__ float alphax(int i)
      {
            ...
            return (abscissa_normalized>=0)?alpha0 * (1.0 - abscissa_normalized):0.0;
      }
      ...
      {
            float abscissa_normalized = MAX((ixe_min - i - 0.5)/(float)DELTA, (i - (ixs_max-1) + 0.5)/(float)DELTA);

            return (abscissa_normalized>=0)?dump0*powf(abscissa_normalized,NPOWER):0.0;
      }
      inline __device__ float dumpy2(int i)
      {
            float abscissa_normalized = MAX((iye_min - i - 0.5)/(float)DELTA, (i - (iys_max-1) + 0.5)/(float)DELTA);

            return (abscissa_normalized>=0)?dump0*powf(abscissa_normalized,NPOWER):0.0;
      }
      inline __device__ float dumpz2(int i)
      {
            float abscissa_normalized = (DELTA - i - 0.5)/(float)DELTA;
         ...
         // pour k = 0 -------------------------------------------------------------------------------------------->>>
            ...
            if (active) {/*{{{*/
               ...
               if ( distance_ymax >= 1 && distance_xmin >= 1 ){
                  float muy = 2.0/(1.0/s_mu[MUOFF(0,0)][0]  + 1.0/s_mu[MUOFF(0,1)][0]);
                  float vpy = 2.0/(1.0/s_vp[VPOFF(0,0)][0] +  1.0/s_vp[VPOFF(0,1)][0]);
               ...
               if (distance_xmin >= 1 ){
                  ...
                  float muz = 2.0/(1.0/s_mu[MUOFF(0,0)][0]  + 1.0/s_mu[MUOFF(0,0)][1]);
                  float vpz = 2.0/(1.0/s_vp[VPOFF(0,0)][0]  + 1.0/s_vp[VPOFF(0,0)][1]);
               ...
               if (distance_ymax >= 1){
                  ...
                  float muxyz = 8.0/(1.0/s_mu[MUOFF(0,0)][0] + 1.0/s_mu[MUOFF(0,0)][1]
                            + 1.0/s_mu[MUOFF(0,1)][0] + 1.0/s_mu[MUOFF(0,1)][1]
                            + 1.0/s_mu[MUOFF(1,0)][0] + 1.0/s_mu[MUOFF(1,0)][1]
                            + 1.0/s_mu[MUOFF(1,1)][0] + 1.0/s_mu[MUOFF(1,1)][1]);
                  float vpxyz = 8.0/(1.0/s_vp[VPOFF(0,0)][0] + 1.0/s_vp[VPOFF(0,0)][1]
                            + 1.0/s_vp[VPOFF(0,1)][0] + 1.0/s_vp[VPOFF(0,1)][1]
                            + 1.0/s_vp[VPOFF(1,0)][0] + 1.0/s_vp[VPOFF(1,0)][1]
                            + 1.0/s_vp[VPOFF(1,1)][0] + 1.0/s_vp[VPOFF(1,1)][1]);
            ...
            for (k = 1; k < delta; k++) {/*{{{*/
               ...
               if (active) {/*{{{*/
                  ...
                  if (distance_ymin >= 1 && distance_xmax >= 1 ){
                     float mux = 2.0/(1.0/s_mu[MUOFF(0,0)][0]  + 1.0/s_mu[MUOFF(1,0)][0]);
                     float vpx = 2.0/(1.0/s_vp[VPOFF(0,0)][0]  + 1.0/s_vp[VPOFF(1,0)][0]);
                     float lamx = 2.0/(1.0/s_lam[LAMOFF(0,0)]  + 1.0/s_lam[LAMOFF(1,0)]);
