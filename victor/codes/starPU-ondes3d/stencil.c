...
int main(int argc, char **argv)
{
   ...
   DGN_DBG
	...
	struct timespec pustart;
	gettime(pustart);
	for (int worker = 0; worker<STARPU_NMAXWORKERS; worker++) {
		pu_last_use[worker] = pustart;
	}
	...
	draw_svg("schedule_time_line.svg");
	...
	if (rank == 0)
	{
		...
		fprintf(stdout, "idle time:\n");
		for (int worker = 0; worker < STARPU_NMAXWORKERS; worker++)
		{
			if (veloc_update_per_worker[worker] || stress_update_per_worker[worker])
			{
				char name[32];
				starpu_worker_get_name(worker, name, sizeof(name));
				fprintf(stdout,"\t%s -> %.2f ms\t\t(%2.2f%%)\n", name, time_spent_idle[worker], 100.0*(time_spent_idle[worker]/(max_timing/1000.0)));
			}
		}
#endif
#if 0
