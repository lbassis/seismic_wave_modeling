...
// SIMULATION INITIALIZATION {{{
	...
	void set_material_properties(ondes3d_params* par, int rank) 
	{
							...
							for (int j=0; j<block->pitchy; j++) {
					   			for (int i=0; i<block->pitchx; i++) {
					   				if (indomain(block, i, j, k, true)) {
										...
										if  (MIN(par->rho0[0], vsdum)==0)
											fprintf(stdout, "RHO zero!!!!\t");
		...
		/*				for (int j=0; j<block->pitchy; j++) 
							for (int i=0; i<block->pitchx; i++)
								if  (block->rho[RAW_ACCESS(block, i,j,k)]==0)
									fprintf(stdout, "RHO zero in bx: %d by: %d i: %d j: %d k: %d \n", bx, by, i, j, k);
								//else
//									fprintf(stdout,"RHO: %f \t",  block->rho[RAW_ACCESS(block, i,j,k)]);*/
	   				}	
	      			}
	      		}
	   	}
	   ...
	   {  for (by = 0; by < nby; by++)
	      {
	      	...
	      	if (node == rank) {
	      		...
	      		for (int k=-K; k<DELTA; k++) { /*zmin -> every block*/
					...
					// 			abscissa_normalized = (DELTA>0)?(ixe_min - i)/(float)DELTA:-1.0f;
					...
					// 			abscissa_normalized = (DELTA>0)?(ixe_min - i - 0.5)/(float)DELTA:-1.0f;
					// 			if (abscissa_normalized >= 0.0) {
					...
					// 			abscissa_normalized = (DELTA>0)?(i - (ixs_max-1)/(float)DELTA:-1.0f;
					// 			if (abscissa_normalized >= 0.0) {
					// 				block->dumpx[i] = par->dump0 * powf(abscissa_normalized,NPOWER);
					// 				block->kappax[i] = 1.0 + (par->kappa0 - 1.0) * powf(abscissa_normalized,NPOWER);
					// 				block->alphax[i] = par->alpha0 * (1.0 - abscissa_normalized);
					...
					// 			abscissa_normalized = (DELTA>0)?(i - (ixs_max-1) + 0.5)/(float)DELTA:-1.0f;
					// 			if (abscissa_normalized >= 0.0) {
					// 				block->dumpx2[i] = par->dump0 * powf(abscissa_normalized,NPOWER);
					// 				block->kappax2[i] = 1.0 + (par->kappa0 - 1.0) * powf(abscissa_normalized,NPOWER);
					// 				block->alphax2[i] = par->alpha0 * (1.0 - abscissa_normalized);
					...
					// 			abscissa_normalized = (DELTA>0)?(iye_min - i)/(float)DELTA:-1.0f;
					// 			if (abscissa_normalized >= 0.0) {
					...
					// 			abscissa_normalized = (DELTA>0)?(iye_min - i - 0.5)/(float)DELTA:-1.0f;
					// 			if (abscissa_normalized >= 0.0) {
					...
					// 			abscissa_normalized = (DELTA>0)?(i - (iys_max-1)/(float)DELTA:-1.0f;
					// 			if (abscissa_normalized >= 0.0) {
					// 				block->dumpy[i] = par->dump0 * powf(abscissa_normalized,NPOWER);
					// 				block->kappay[i] = 1.0 + (par->kappa0 - 1.0) * powf(abscissa_normalized,NPOWER);
					// 				block->alphay[i] = par->alpha0 * (1.0 - abscissa_normalized);
					...
					// 			abscissa_normalized = (DELTA>0)?(i - (iys_max-1) + 0.5)/(float)DELTA:-1.0f;
					// 			if (abscissa_normalized >= 0.0) {
					// 				block->dumpy2[i] = par->dump0 * powf(abscissa_normalized,NPOWER);
					// 				block->kappay2[i] = 1.0 + (par->kappa0 - 1.0) * powf(abscissa_normalized,NPOWER);
					// 				block->alphay2[i] = par->alpha0 * (1.0 - abscissa_normalized);
					...
					// 	abscissa_normalized = (DELTA>0)?(DELTA - i)/(float)DELTA:-1.0f;
					// 	if (abscissa_normalized >= 0.0) {
					...
					// 	abscissa_normalized = (DELTA>0)?(DELTA - i - 0.5)/(float)DELTA:-1.0f;
					// 	if (abscissa_normalized >= 0.0) {
...
/// CODE CHANGED{{{
/*			abscissa_normalized = (DELTA>0)?(DELTA - k)/(float)DELTA:-1.0;
			int distance_zmin = k;
...
/// CODE CHANGED {{{
/*				abscissa_normalized = (DELTA>0)?MAX((iye_min - j)/(float)DELTA, (j - (iys_max-1)/(float)DELTA)):-1.0;
...
/// }}}
				for ( int i = 0; i < sizex; i++ ){
					...
					fx[offset] = (float)rand()/(float)RAND_MAX*1000;
					fy[offset] = (float)rand()/(float)RAND_MAX*1000;
					fz[offset] = (float)rand()/(float)RAND_MAX*1000;
	//				fprintf(f_vx, "%d\t %d\t %d\t\t VX0 before:%10.12f \t", i, j, k, vx0[offset]);
		//			fprintf(f_vx, "RHO: %f\t FX: %f\t TXX0: %f\t TXY0: %f\t TXZ0: %f\n", rho[offset], fx[offset], txx0[offset], txy0[offset], txz0[offset]);
					if (rho[offset] != 0) {
			//			rho[offset] = ((float)rand()/RAND_MAX)*10000;
				
/// NEW CODE {{{		
		//				fprintf(stdout,  "RHO: %f\t",  rho[offset]);
					
						//Calculate Vx
						vx0[offset] += (1./rho[offset])*fx[offset]*dt/ds
							+ (9*(1./rho[offset])*dt/8)*((txx0[offset] - txx0[OFFSET_PITCH(pitch_x, pitch_y, i, j, k-1)])
							+ (txy0[offset] - txy0[OFFSET_PITCH(pitch_x, pitch_y, i, j-1, k)])
							+ (txz0[offset] - txz0[OFFSET_PITCH(pitch_x, pitch_y, i-1, j, k)]))
							- ((1./rho[offset])*dt/24)*((txx0[OFFSET_PITCH(pitch_x, pitch_y, i, j, k+1)] - txx0[OFFSET_PITCH(pitch_x, pitch_y, i, j, k-2)])
							+ (txy0[OFFSET_PITCH(pitch_x, pitch_y, i, j+1, k)] - txy0[OFFSET_PITCH(pitch_x, pitch_y, i, j-2, k)])
							+ (txz0[OFFSET_PITCH(pitch_x, pitch_y, i+1, j, k)] - txz0[OFFSET_PITCH(pitch_x, pitch_y, i-2, j, k)]));

						//Calculate Vy
						vy0[offset] += (1./rho[offset])*fy[offset]*dt/ds
							+ (9*(1./rho[offset])*dt/8)*((txy0[OFFSET_PITCH(pitch_x, pitch_y, i, j, k+1)] - txy0[offset])
							+ (tyy0[OFFSET_PITCH(pitch_x, pitch_y, i, j+1, k)] - tyy0[OFFSET_PITCH(pitch_x, pitch_y, i, j, k)])
							+ (tyz0[offset] - tyz0[OFFSET_PITCH(pitch_x, pitch_y, i-1, j, k)]))
							- ((1./rho[offset])*dt/24)*((txy0[OFFSET_PITCH(pitch_x, pitch_y, i, j, k+2)] - txy0[OFFSET_PITCH(pitch_x, pitch_y, i, j, k-1)])
							+ (tyy0[OFFSET_PITCH(pitch_x, pitch_y, i, j+2, k)] - tyy0[OFFSET_PITCH(pitch_x, pitch_y, i, j-1, k)])
							+ (tyz0[OFFSET_PITCH(pitch_x, pitch_y, i+1, j, k)] -  tyz0[OFFSET_PITCH(pitch_x, pitch_y, i-2, j, k)]));

						//Calculta Vz
						vz0[offset] += (1./rho[offset])*fz[offset]*dt/ds
							+ (9*(1./rho[offset])*dt/8)*((txz0[OFFSET_PITCH(pitch_x, pitch_y, i, j, k+1)] -  txz0[offset])
							+ (tyz0[offset] - tyz0[OFFSET_PITCH(pitch_x, pitch_y, i, j-1, k)])
							+ (tzz0[OFFSET_PITCH(pitch_x, pitch_y, i+1, j, k)] -  tzz0[offset]))
							- ((1./rho[offset])*dt/24)*((txz0[OFFSET_PITCH(pitch_x, pitch_y, i, j, k+2)] -  txz0[OFFSET_PITCH(pitch_x, pitch_y, i, j, k-1)])
							+ (tyz0[OFFSET_PITCH(pitch_x, pitch_y, i, j+1, k)] - tyz0[OFFSET_PITCH(pitch_x, pitch_y, i, j-2, k)])
							+ (tzz0[OFFSET_PITCH(pitch_x, pitch_y, i+2, j, k)] - tzz0[OFFSET_PITCH(pitch_x, pitch_y, i-1, j, k)]));
/// }}}
/// CODE CHANGED {{{
/*
					// vx0 init  random
					//vx0[offset]+=(float)rand()/((float)RAND_MAX);
					//fprintf(f_vx, "%d\t %d\t %d\t\t VX0 before:%10.12f\n", i, j, k, vx0[offset]);
					int npml = npml_tab[k*(sizex)*(sizey) + j*(sizex) + i];
					if (npml >= sizex*sizey*sizez) {
						printf("STOP\n");
					}
					...
					if (npml >= 0) {// CPML
// DGN DEBUG
	            				abscissa_normalized = (DELTA>0)?MAX((ixe_min - i)/(float)DELTA, (i - (ixs_max-1)/(float)DELTA)):-1.0;
...
//										FILE* f_ts)
	{
		...
		for ( int k = 0; k < sizez; k++) {
/// CODE CHANGED {{{
/*			abscissa_normalized = (DELTA>0)?(DELTA - k)/(float)DELTA:-1.0f;
			int distance_zmin = k;
			int distance_zmax = sizez - 1 - k;
...
/// }}}
			for ( int j = 0; j < sizey; j++ ){
/// CODE CHANGED {{{
/*				abscissa_normalized = (DELTA>0)?MAX((iye_min - j)/(float)DELTA, (j - (iys_max-1)/(float)DELTA)):-1.0f;
				int distance_ymin = (position & MASK_FIRST_Y)?j:DUMMY_VALUE;
				int distance_ymax = (position & MASK_LAST_Y)?(sizey - j - 1):DUMMY_VALUE;
...
/// NEW CODE {{{
					if (vp[offset] != 0) {
						lamx = (float)rand()/(float)RAND_MAX*1000;
						mux = (float)rand()/(float)RAND_MAX*1000;
						muy = (float)rand()/(float)RAND_MAX*1000;
						muz = (float)rand()/(float)RAND_MAX*1000;
						muxyz = (float)rand()/(float)RAND_MAX*1000;
						txx0[offset] +=
							(9*dt/8)*((lamx+2*mux)*(vx0[OFFSET_PITCH (pitch_x, pitch_y, i, j, k+1)] - vx0[offset])
							+  lamx*(vy0[offset] - vy0[OFFSET_PITCH (pitch_x, pitch_y, i, j-1, k)])
							+  lamx*(vz0[offset] - vz0[OFFSET_PITCH (pitch_x, pitch_y, i-1, j, k)]))/ds 
							- (dt/24)*( (lamx+2*mux)*(vx0[OFFSET_PITCH (pitch_x, pitch_y, i, j, k+2)] - vx0[OFFSET_PITCH (pitch_x, pitch_y, i, j, k-1)])
							+  lamx*(vy0[OFFSET_PITCH (pitch_x, pitch_y, i, j+1, k)] - vy0[OFFSET_PITCH (pitch_x, pitch_y, i, j-2, k)]) 
							+  lamx*(vz0[OFFSET_PITCH (pitch_x, pitch_y, i+1, j, k)] - vz0[OFFSET_PITCH (pitch_x, pitch_y, i-2, j, k)]))/ds;

				                tyy0[offset] +=
					                (9*dt/8)*( (lamx+2*mux)*(vy0[offset] - vy0[OFFSET_PITCH (pitch_x, pitch_y, i, j-1, k)])
					                +  lamx*(vx0[OFFSET_PITCH (pitch_x, pitch_y, i, j, k+1)] - vx0[offset]         ) 
					                +  lamx*(vz0[offset] - vz0[OFFSET_PITCH (pitch_x, pitch_y, i-1, j, k)] ))/ds
					                - (dt/24)*( (lamx+2*mux)*(vy0[OFFSET_PITCH (pitch_x, pitch_y, i, j+1, k)] - vy0[OFFSET_PITCH (pitch_x, pitch_y, i, j-2, k)])
					                +  lamx*(vx0[OFFSET_PITCH (pitch_x, pitch_y, i, j, k+2)] - vx0[OFFSET_PITCH (pitch_x, pitch_y, i, j, k-1)])
					                +  lamx*(vz0[OFFSET_PITCH (pitch_x, pitch_y, i+1, j, k)] -vz0[OFFSET_PITCH (pitch_x, pitch_y, i-2, j, k)] ))/ds;

						tzz0[offset] +=
					                (9*dt/8)*( (lamx+2*mux)*(vz0[offset] - vz0[OFFSET_PITCH (pitch_x, pitch_y, i-1, j, k)]  )
					                +  lamx*(vx0[OFFSET_PITCH (pitch_x, pitch_y, i, j, k+1)] -  vx0[offset] )
					                +  lamx*(vy0[offset] - vy0[OFFSET_PITCH (pitch_x, pitch_y, i, j-1, k)] ))/ds
					                - (dt/24)*( (lamx+2*mux)*(vz0[OFFSET_PITCH (pitch_x, pitch_y, i+1, j, k)] - vz0[OFFSET_PITCH (pitch_x, pitch_y, i-2, j, k)] )
					                +  lamx*(vx0[OFFSET_PITCH (pitch_x, pitch_y, i, j, k+2)] - vx0[OFFSET_PITCH (pitch_x, pitch_y, i, j, k-1)] )
					                +  lamx*(vy0[OFFSET_PITCH (pitch_x, pitch_y, i, j+1, k)]) - vy0[OFFSET_PITCH (pitch_x, pitch_y, i, j-2, k)] )/ds;

				                txy0[offset] += 
							   (9*dt*muy/8)*(( vy0[offset] - vy0[OFFSET_PITCH (pitch_x, pitch_y, i, j, k-1)]  ) 
					                   + (vx0[OFFSET_PITCH (pitch_x, pitch_y, i, j+1, k)] - vx0[OFFSET_PITCH (pitch_x, pitch_y, i, j, k)] ))/ds
					      		   - (dt*muy/24)*((vy0[OFFSET_PITCH (pitch_x, pitch_y, i, j, k+1)] - vy0[OFFSET_PITCH (pitch_x, pitch_y, i, j, k-2)] ) 
					                   +  (vx0[OFFSET_PITCH (pitch_x, pitch_y, i, j+2, k)] - vx0[OFFSET_PITCH (pitch_x, pitch_y, i, j-1, k)]))/ds;

				                txz0[offset] +=
					                   (9*dt*muz/8)*((vz0[offset] - vz0[OFFSET_PITCH (pitch_x, pitch_y, i, j, k-1)]   )
					                   + (vx0[OFFSET_PITCH (pitch_x, pitch_y, i+1, j, k)] - vx0[offset] ))/ds
					                   - (dt*muz/24)*((vz0[offset] - vz0[OFFSET_PITCH (pitch_x, pitch_y, i, j, k-2)]  )
					                   +  (vx0[OFFSET_PITCH (pitch_x, pitch_y, i+2, j, k)] - vx0[OFFSET_PITCH (pitch_x, pitch_y, i-1, j, k)] ))/ds;

				                tyz0[offset] +=
					                   (9*dt*muxyz/8)*((vz0[OFFSET_PITCH (pitch_x, pitch_y, i, j+1, k)] - vz0[offset]   )
					                   + (vy0[OFFSET_PITCH (pitch_x, pitch_y, i+1, j, k)] - vy0[offset] ))/ds
					                   - (dt*muxyz/24)*((vz0[OFFSET_PITCH (pitch_x, pitch_y, i, j+1, k)] - vz0[offset]  )
					                   +  (vy0[OFFSET_PITCH (pitch_x, pitch_y, i+2, j, k)] - vy0[OFFSET_PITCH (pitch_x, pitch_y, i-1, j, k)] ))/ds;

					}
	//				 fprintf(f_ts, "%d\t %d\t %d\t\t %10.12f\t %10.12f\t %10.12f\t %10.12f\t %10.12f\t %10.12f\n", i, j, k, txx0[offset], tyy0[offset], tzz0[offset],  txy0[offset], txz0[offset], tyz0[offset]);
///}}}
...
/*
					...
					int distance_xmin = (position & MASK_FIRST_X)?i:DUMMY_VALUE;
					int distance_xmax = (position & MASK_LAST_X)?(sizex - i - 1):DUMMY_VALUE;
//					fprintf(stdout, "VX: %f\t", vx0[offset]);

					if (npml >= 0) {// CPML 
// if (!k) DGN_DBG
						abscissa_normalized = (DELTA>0)?MAX((ixe_min - i)/(float)DELTA, (i - (ixs_max-1)/(float)DELTA)):-1.0f;
						float dumpx_  = (abscissa_normalized>=0)?dump0*powf(abscissa_normalized,NPOWER):0.0;
						float kappax_ = (abscissa_normalized>=0)?1.0 + (kappa0 - 1.0) * powf(abscissa_normalized,NPOWER):1.0;
						float alphax_ = (abscissa_normalized>=0)?alpha0 * (1.0 - abscissa_normalized):0.0;
						float dumpx2_ = (abscissa_normalized>=0)?dump0*powf(abscissa_normalized,NPOWER):0.0;
