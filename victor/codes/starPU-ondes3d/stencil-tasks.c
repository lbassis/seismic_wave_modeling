...
// DGN : a mettre dans les versions MPI si besoin ...
  if (iter == get_niter()-1)
  {
    save_task->detach = 0;
    save_task->use_tag = 1;
    save_task->tag_id = TAG_FINISH(x,y);
  }
#endif
...
void create_task_compute_veloc(unsigned iter, unsigned x, unsigned y, unsigned local_rank)
{
	...
	task->handles[9] = descr->force_handle[0];
	task->handles[10] = descr->force_handle[1];
	task->handles[11] = descr->force_handle[2];

	task->handles[12] = descr->rho_handle;
	task->handles[13] = descr->vp_handle;

	task->handles[14] = descr->npml_tab_handle;

	task->handles[15] = descr->phit_handle[0];
	task->handles[16] = descr->phit_handle[1];
	task->handles[17] = descr->phit_handle[2];
	task->handles[18] = descr->phit_handle[3];
	task->handles[19] = descr->phit_handle[4];
	task->handles[20] = descr->phit_handle[5];
	task->handles[21] = descr->phit_handle[6];
	task->handles[22] = descr->phit_handle[7];
	task->handles[23] = descr->phit_handle[8];
#endif
...
#ifdef VERSION_SIMPLIFIEE
  /* We are going to synchronize with the last tasks */
	// cas dégradé avec un bloc unique
  if (get_nbx()==1 && get_nby() == 1 && iter == get_niter()-1)
  {
    task->detach = 0;
    task->use_tag = 1;
...
void create_start_task(int x, int y, int dir)
{
	...
	wait_init->handles[9] = descr->velocity_handle[0];
	wait_init->handles[10] = descr->velocity_handle[1];
	wait_init->handles[11] = descr->velocity_handle[2];

	wait_init->handles[12] = descr->stress_handle[0];
	wait_init->handles[13] = descr->stress_handle[1];
	wait_init->handles[14] = descr->stress_handle[2];
	wait_init->handles[15] = descr->stress_handle[3];
	wait_init->handles[16] = descr->stress_handle[4];
	wait_init->handles[17] = descr->stress_handle[5];

	wait_init->handles[18] = descr->force_handle[0];
	wait_init->handles[19] = descr->force_handle[1];
	wait_init->handles[20] = descr->force_handle[2];
...
#define DEPDIST 2
	// nb diagonals
	int nb_diag = get_nbx() + get_nby() -1;
	int nb_step = nb_diag*niter + DEPDIST;

	for (int step = 0; step < nb_step; step++) {
	//	fprintf(stdout, "Skewing nbx %d nby %d iter %d \n", nbx, nby, step); 
		int diag0 = step%nb_diag;
		int diag1 = (step - DEPDIST)%nb_diag;
		int iter0 = step/nb_diag;
		int iter1 = (step - DEPDIST)/nb_diag;
		for(int task_set = 0; task_set < 2; task_set++) {
			if (task_set == 0) {
				for (bx = 0; bx < nbx; bx++) {
					for (by = 0; by < nby; by++) {
	//					fprintf(stdout, "Skewing bx %d by %d set %d \n", bx, by, task_set); 
						if (bx+by == diag0 && iter0 < niter) {
							// create taskset0 for block {bx,by}
							struct block_description * block = get_block_description(bx, by);

#ifndef VERSION_SIMPLIFIEE
							if (block->mpi_node == rank && block->nb_src > 0){
								fprintf(stdout,"Updating source \n");
								create_task_update_sources(iter0, bx, by, rank);}
#endif
						    	if (block->mpi_node == rank) {
								//create_task_update_sources(iter0, bx, by, rank);
					      			create_task_compute_veloc(iter0, bx, by, rank);
}
					    		if ((block->mpi_node == rank) || (get_block_mpi_node(bx+1, by) == rank)) 
								create_task_save(iter0, bx, by, XP, rank, VELOC);
							if ((block->mpi_node == rank) || (get_block_mpi_node(bx-1, by) == rank))
								create_task_save(iter0, bx, by, XM, rank, VELOC);
							if ((block->mpi_node == rank) || (get_block_mpi_node(bx, by+1) == rank))
								create_task_save(iter0, bx, by, YP, rank, VELOC);
							if ((block->mpi_node == rank) || (get_block_mpi_node(bx, by-1) == rank))
								create_task_save(iter0, bx, by, YM, rank, VELOC);
				...
				for (bx = 0; bx < nbx; bx++) {
					for (by = 0; by < nby; by++) {
	//					fprintf(stdout, "Skewing bx %d by %d \n", bx, by); 
						if (bx+by == diag1) {
							// create taskset1 for block {bx,by}
							struct block_description * block = get_block_description(bx, by);
							if (block->mpi_node == rank)
								create_task_compute_stress(iter1, bx, by, rank);
							if ((block->mpi_node == rank) || (get_block_mpi_node(bx+1, by) == rank)) 
								create_task_save(iter1, bx, by, XP, rank, STRESS);
							if ((block->mpi_node == rank) || (get_block_mpi_node(bx-1, by) == rank))
								create_task_save(iter1, bx, by, XM, rank, STRESS);
							if ((block->mpi_node == rank) || (get_block_mpi_node(bx, by+1) == rank))
								create_task_save(iter1, bx, by, YP, rank, STRESS);
							if ((block->mpi_node == rank) || (get_block_mpi_node(bx, by-1) == rank))
								create_task_save(iter1, bx, by, YM, rank, STRESS);
#ifndef VERSION_SIMPLIFIEE
							if (block->mpi_node == rank)
								create_task_record_seismo(iter1, bx, by, rank);
#endif
						}
					}	
				}
	...
	{  
		...
		{  	
			for (by = 0; by < nby; by++)
	      		{
	//			fprintf(stdout, "No skewing update resource bx %d by %d iter %d \n", bx, by, iter); 
			      	struct block_description * block = get_block_description(bx, by);
		...
		for (bx = 0; bx < nbx; bx++)
	   	{
			for (by = 0; by < nby; by++)
	      		{
	//			fprintf(stdout, "No skewing compute velocity bx %d by %d iter %d \n", bx, by, iter); 
			      	struct block_description * block = get_block_description(bx, by);
	      			// II) Kernel_1 : V = f(T)
		      		if (block->mpi_node == rank) {

								fprintf(stdout, "linea 859 \n");
		...
		for (bx = 0; bx < nbx; bx++)
	   	{
			for (by = 0; by < nby; by++)
	      		{
	//			fprintf(stdout, "No skewing save bx %d by %d iter %d \n", bx, by, iter); 
	      			struct block_description * block = get_block_description(bx, by);
