...
float time_spent_idle[STARPU_NMAXWORKERS];
float time_spent_idle[STARPU_NMAXWORKERS];
struct timespec pu_last_use[STARPU_NMAXWORKERS];
...
// compute_veloc_func_cuda {{{
	...
	static void compute_veloc_func_cuda(void *descr[], void *arg)
	{
		...
		// 	fprintf(stderr,"!!! DO compute_veloc_cuda block{%d, %d} %s !!!\n", block->bx, block->by, name);
		// else
		char name[7];
		starpu_worker_get_name(workerid, name, 7);
		INFO( "!!! DO compute_veloc_cuda it=%d, block{%d, %d} %s !!!\n", block->iter, block->bx, block->by, name);
		...
		struct starpu_block_interface *b_fx = (struct starpu_block_interface *)descr[9];
		struct starpu_block_interface *b_fy = (struct starpu_block_interface *)descr[10];
		struct starpu_block_interface *b_fz = (struct starpu_block_interface *)descr[11];

		struct starpu_block_interface *b_rho = (struct starpu_block_interface *)descr[12];
		struct starpu_block_interface *b_vp = (struct starpu_block_interface *)descr[13];

		struct starpu_block_interface *b_npml = (struct starpu_block_interface *)descr[14];

		struct starpu_block_interface *b_phitxxx = (struct starpu_block_interface *)descr[15];
		struct starpu_block_interface *b_phitxyy = (struct starpu_block_interface *)descr[16];
		struct starpu_block_interface *b_phitxzz = (struct starpu_block_interface *)descr[17];
		struct starpu_block_interface *b_phitxyx = (struct starpu_block_interface *)descr[18];
		struct starpu_block_interface *b_phityyy = (struct starpu_block_interface *)descr[19];
		struct starpu_block_interface *b_phityzz = (struct starpu_block_interface *)descr[20];
		struct starpu_block_interface *b_phitxzx = (struct starpu_block_interface *)descr[21];
		struct starpu_block_interface *b_phityzy = (struct starpu_block_interface *)descr[22];
		struct starpu_block_interface *b_phitzzz = (struct starpu_block_interface *)descr[23];
#endif
	...
	//	FILE *f_vx; char filename[64];
	//	fprintf(stdout, "Creating file VXYZ-CUDA for iter %d bx: %d by: %d workerid: %d\n", block->iter, block->bx, block->by, workerid);

	//	sprintf(filename, "vxyz-cuda-iter-%d-bx-%d-by%d-worker-%d.csv", block->iter, block->bx, block->by, workerid);
	//	f_vx = fopen(filename, "w");
	//	fprintf(f_vx, "i\t j\t k\t\t VX\t VY\t VZ\n");
	...
	//	fclose(f_vx);
		time_spent_veloc_update[starpu_worker_get_id()] += msecdur(start,end);
		time_spent_idle[starpu_worker_get_id()] += msecdur(pu_last_use[starpu_worker_get_id()],start);
		gettime(pu_last_use[starpu_worker_get_id()]);
		register_event(workerid, _VELOC, block->bx, block->by, block->iter, start, end);
...
// compute_stress_func_cuda {{{
	...
	// 	if (block->bx == 0 && block->by == 0)
	// fprintf(stderr,"!!! DO compute_stress_cuda block{%d, %d} %s !!!\n", block->bx, block->by, name);
	// 	else
		char name[7];
		starpu_worker_get_name(workerid, name, 7);
		INFO( "!!! DO compute_stress_cuda it=%d, block{%d, %d} %s !!!\n", block->iter, block->bx, block->by, name);
	#ifdef STARPU_USE_MPI
		int rank = 0;
		MPI_Comm_rank(MPI_COMM_WORLD, &rank);
		DEBUG( "!!!                 RANK %d                    !!!\n", rank);
	...
	//		STARPU_CUDA_REPORT_ERROR(cures);
		...
		gettime(end);
		time_spent_stress_update[workerid] += msecdur(start,end);
		time_spent_idle[starpu_worker_get_id()] += msecdur(pu_last_use[starpu_worker_get_id()],start);
		gettime(pu_last_use[starpu_worker_get_id()]);

		register_event(starpu_worker_get_id(), _STRESS, block->bx, block->by, block->iter, start, end);
...
// compute_veloc_func_cpu {{{
	...
	static void compute_veloc_func_cpu(void *descr[], void *arg)
	{
		...
		block->iter++;	
	// 	if (block->bx == 0 && block->by == 0)
	// fprintf(stderr,"!!! DO compute_veloc_cpu block{%d, %d} %s !!!\n", block->bx, block->by, name);
	// 	else
		char name[7];
		starpu_worker_get_name(workerid, name, 7);
		INFO("!!! DO compute_veloc_cpu it=%d, block{%d, %d} %s !!!\n", block->iter, block->bx, block->by, name);
	#ifdef STARPU_USE_MPI
		int rank = 0;
		MPI_Comm_rank(MPI_COMM_WORLD, &rank);
		DEBUG( "!!!                 RANK %d                    !!!\n", rank);
		 ...
		 */
#ifdef VERSION_SIMPLIFIEE
	//	fprintf(stdout, "simplifiee \n");
		 // DGN : fait dans record_seismo normalement
		 /*XP*/
		if (block->bx < get_nbx()-1) {
...
//		fprintf(stdout, "No simplifiee \n");
		...
		//Create file VX
	//	FILE *f_vx; char filename[64];
	//	fprintf(stdout, "Creating file VXYZ-CPU for iter %d bx: %d by: %d workerid: %d\n", block->iter, block->bx, block->by, workerid);

	//	sprintf(filename, "vxyz-cpu-iter-%d-bx-%d-by%d-worker-%d.csv", block->iter, block->bx, block->by, workerid);
	//	f_vx = fopen(filename, "w");
	//	fprintf(f_vx, "i\t j\t k\t\t VX\t VY\t VZ\n");
	...
	//	fclose(f_vx);
		...
		float duration = msecdur(start,end);
		time_spent_veloc_update[starpu_worker_get_id()] += duration;
		time_spent_idle[starpu_worker_get_id()] += msecdur(pu_last_use[starpu_worker_get_id()],start);
		gettime(pu_last_use[starpu_worker_get_id()]);
		register_event(starpu_worker_get_id(), _VELOC, block->bx, block->by, block->iter, start, end);
		DEBUG("<<< compute veloc cpu kernel\tit=%d\t{%d, %d}\tCPU %d\n", block->iter, block->bx, block->by, workerid);
	}
// }}}
...
// compute_stress_func_cpu {{{
	...
	static void compute_stress_func_cpu(void *descr[], void *arg)
	{
		...
		// if (block->bx == 0 && block->by == 0)
		// 	fprintf(stderr,"!!! DO compute_stress_cpu block{%d, %d} %s !!!\n", block->bx, block->by, name);
		// else
		char name[7];
		starpu_worker_get_name(workerid, name, 7);
		INFO("!!! DO compute_stress_cpu it=%d, block{%d, %d} %s !!!\n", block->iter, block->bx, block->by, name);
	#ifdef STARPU_USE_MPI
		int rank = 0;
		MPI_Comm_rank(MPI_COMM_WORLD, &rank);
		DEBUG( "!!!                 RANK %d                    !!!\n", rank);
	...
	//	FILE *f_ts; char filename[64];
//		fprintf(stdout, "Creating file TXYZ-CPU for iter %d bx: %d by: %d workerid: %d\n", block->iter, block->bx, block->by, workerid);

	///	sprintf(filename, "txyz-cpu-iter-%d-bx-%d-by%d-worker-%d.csv", block->iter, block->bx, block->by, workerid);
	//	f_ts = fopen(filename, "w");
	//	fprintf(f_ts, "i\t j\t k\t\t TXX\t TYY\t TZZ\t TXY\t TXZ\t TYZ\n");
	...
	// Save veloc {{{
		...
		static void save_veloc_xp_cpu(void *descr[] __attribute__((unused)), void *arg)
		{
		  ...
		  gettime(end);
			...
			time_spent_idle[starpu_worker_get_id()] += msecdur(pu_last_use[starpu_worker_get_id()],start);
			gettime(pu_last_use[starpu_worker_get_id()]);
			register_event(starpu_worker_get_id(), _V_XP, block->bx, block->by, block->iter, start, end);
		...
		static void save_veloc_xm_cpu(void *descr[] __attribute__((unused)), void *arg)
		{
  		...
  		gettime(end);
			...
			time_spent_idle[starpu_worker_get_id()] += msecdur(pu_last_use[starpu_worker_get_id()],start);
			gettime(pu_last_use[starpu_worker_get_id()]);
			register_event(starpu_worker_get_id(), _V_XM, block->bx, block->by, block->iter, start, end);
		...
		static void save_veloc_yp_cpu(void *descr[] __attribute__((unused)), void *arg)
		{
  		...
  		gettime(end);
			...
			time_spent_idle[starpu_worker_get_id()] += msecdur(pu_last_use[starpu_worker_get_id()],start);
			gettime(pu_last_use[starpu_worker_get_id()]);
			register_event(starpu_worker_get_id(), _V_YP, block->bx, block->by, block->iter, start, end);
		...
		static void save_veloc_ym_cpu(void *descr[] __attribute__((unused)), void *arg)
		{
		  ...
		  gettime(end);
			...
			time_spent_idle[starpu_worker_get_id()] += msecdur(pu_last_use[starpu_worker_get_id()],start);
			gettime(pu_last_use[starpu_worker_get_id()]);
			register_event(starpu_worker_get_id(), _V_YM, block->bx, block->by, block->iter, start, end);
	...
	// Save stress {{{
		...
		static void save_stress_xp_cpu(void *descr[] __attribute__((unused)), void *arg)
		{
		  ...
		  gettime(end);
			...
			time_spent_idle[starpu_worker_get_id()] += msecdur(pu_last_use[starpu_worker_get_id()],start);
			gettime(pu_last_use[starpu_worker_get_id()]);
			register_event(starpu_worker_get_id(), _S_XP, block->bx, block->by, block->iter, start, end);
		...
		static void save_stress_xm_cpu(void *descr[] __attribute__((unused)), void *arg)
		{
		  ...
		  gettime(end);
			...
			time_spent_idle[starpu_worker_get_id()] += msecdur(pu_last_use[starpu_worker_get_id()],start);
			gettime(pu_last_use[starpu_worker_get_id()]);
			register_event(starpu_worker_get_id(), _S_XM, block->bx, block->by, block->iter, start, end);
		...
		static void save_stress_yp_cpu(void *descr[] __attribute__((unused)), void *arg)
		{
		  ...
		  gettime(end);
			...
			time_spent_idle[starpu_worker_get_id()] += msecdur(pu_last_use[starpu_worker_get_id()],start);
			gettime(pu_last_use[starpu_worker_get_id()]);
			register_event(starpu_worker_get_id(), _S_YP, block->bx, block->by, block->iter, start, end);
		...
		static void save_stress_ym_cpu(void *descr[] __attribute__((unused)), void *arg)
		{
		  ...
		  gettime(end);
			...
			time_spent_idle[starpu_worker_get_id()] += msecdur(pu_last_use[starpu_worker_get_id()],start);
			gettime(pu_last_use[starpu_worker_get_id()]);
			register_event(starpu_worker_get_id(), _S_YM, block->bx, block->by, block->iter, start, end);
...
// Sources update {{{
	...
	{
					DGN_DBG
		fprintf(stdout,"Updating surces cpu \n");
	...
	// 	if (block->bx == 0 && block->by == 0)
	// fprintf(stderr,"!!! DO update_sources_cpu block{%d, %d} %s !!!\n", block->bx, block->by, name);
	// 	else
		char name[7];
		starpu_worker_get_name(workerid, name, 7);
		INFO("!!! DO update_sources_cpu it=%d, block{%d, %d} %s !!!\n", block->iter, block->bx, block->by, name);
	#ifdef STARPU_USE_MPI
		int rank = 0;
		MPI_Comm_rank(MPI_COMM_WORLD, &rank);
		DEBUG( "!!!                 RANK %d                    !!!\n", rank);
	...
	/*		record_who_runs_what(block);*/
         ...
         for (int is = 0; is < par->nb_src; is++ ){/*{{{*/
            if ( block->insrc[is] ){
               ...
               for ( unsigned iw = 0; iw < 8; iw++ ){
                  ...
                  // update source array
                  //
                  fprintf(stdout, "Updating source fx \n");
      ...
      free(arg);
		  gettime(end);
			register_event(starpu_worker_get_id(), _SOURCE, block->bx, block->by, block->iter, start, end);
	}
...
// Seismo record {{{
	void record_seismo_cpu(void *descr[], void *arg_)
	{
		...
		// 
	// 	if (block->bx == 0 && block->by == 0)
	// fprintf(stderr,"!!! DO record_seismo_cpu block{%d, %d} %s !!!\n", block->bx, block->by, name);
	// 	else
		...
		INFO("!!! DO record_seismo_cpu it=%d, block{%d, %d} %s !!!\n", block->iter, block->bx, block->by, name);
		...
		gettime(end);
		register_event(workerid, _SISMO, block->bx, block->by, block->iter, start, end);
	}
